...
def plot_aldp_free_energy(
    ...
    for x, label in zip(x_positions, labels):
        fig.text(x, y_pos, label, va="top", ha="left")
    ...
    ax_phi.plot(
        x_grid, F_phi_rew, color=palette_paper["Proposal (reweighted)"], linestyle="--", label="CG-BG (reweighted)"
    )
    ax_phi.set_xlabel(r"$\phi$")
    ...
    ax_psi.plot(
        x_grid, F_psi_rew, color=palette_paper["Proposal (reweighted)"], linestyle="--", label="CG-BG (reweighted)"
    )
    ax_psi.set_xlabel(r"$\psi$")
...
def plot_aldp_ramachandran_fes(
    ...
    ax[0].pcolormesh(x_ref, y_ref, h_ref.T, cmap=cmap, vmax=5.25)
    ...
    ax[1].pcolormesh(x, y, h.T, cmap=cmap, vmax=5.25)
    ...
    h = -kT * jnp.log(h)
    x, y = jnp.meshgrid(x_edges, y_edges)
    ax[2].pcolormesh(x, y, h.T, cmap=cmap, vmax=5.25)
...
def aldp_plots(reweighted_state: dict, task: str, ref_dir: str):

    kT = quantity.kb * 300  # in kJ/mol
    ref_dir = Path(ref_dir)

    sample_impl_path = ref_dir / "openmm_allatomMD_implicit.npz"
    phi_impl_indices = [4, 6, 7, 8]
    psi_impl_indices = [6, 7, 8, 16]

    if "CB" in task:
        sample_ref_path = ref_dir / "openmm_corebetaMD.npz"
        phi_indices = [0, 1, 2, 4]
        psi_indices = [1, 2, 4, 5]
    elif "HA" in task:
        sample_ref_path = ref_dir / "openmm_heavyatomMD.npz"
        phi_indices = [1, 3, 4, 6]
        psi_indices = [3, 4, 6, 8]

    log_w = reweighted_state["log_w"]
    mask = log_w < jnp.percentile(log_w, 99)
    log_w_masked = jnp.where(mask, log_w, -jnp.inf)
    log_w_masked = log_w_masked - jax.scipy.special.logsumexp(log_w_masked)
    w = jnp.exp(log_w_masked)
...
def mb_plots(reweighted_state: dict, ref_dir: str):
    kT = 1.0
    ref_dir = Path(ref_dir)
    ...
    sample_ref_path = ref_dir / "mb_unbiased.npz"
    data_ref = jnp.load(sample_ref_path, allow_pickle=True)
